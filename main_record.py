...
if recording_type == "1":
    ...
    with open("info_openrec_"+datetime.now().strftime("%d%m%Y_%H%M%S")+".csv", "w", newline="") as csvfile:
...
# timed recording
    ...
    # get some information about the recording and save it to csv
    with open("info_timedrec_"+datetime.now().strftime("%d%m%Y_%H%M%S")+".csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        filling = True
        while filling:
            impedance =     input("\nImpedance:                                    ")
