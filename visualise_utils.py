...
class DataInlet(Inlet):
    ...
    def pull_and_plot(self, plot_time, plt):
        ...
        if ts:
            ...
            print(f"Pulled samples: {y}")  # Print the pulled samples
            this_x = None
            old_offset = 0
            new_offset = 0
...
def display_data(listen_to_streams: List[str]):
    ...
    for info in streams:
        # lcheck if info.type matches any of the streams we want to plot
        if (
            info.type() in listen_to_streams
            and info.nominal_srate() != pylsl.IRREGULAR_RATE
